...
export function useAdminActions() {
  ...
  return {
    ...
    verifyCompany: useMutation({ mutationFn: (id: string) => companyApi.verify(id), onSuccess: async () => { await refreshAdmin(); completed("The company verification was approved."); }, onError: failed }),
    ...
    updateJob: useMutation({ mutationFn: ({ id, payload }: { id: string; payload: Record<string, unknown> }) => jobsApi.update(id, payload), onSuccess: async () => { await refreshAdmin(); completed("The job was updated."); }, onError: failed }),
    deleteJob: useMutation({ mutationFn: (id: string) => jobsApi.delete(id), onSuccess: async () => { await refreshAdmin(); completed("The job was deleted."); }, onError: failed }),
    ...
    bulkVerifyCompanies: useMutation({ mutationFn: (ids: string[]) => Promise.all(ids.map((id) => companyApi.verify(id))), onSuccess: async () => { await refreshAdmin(); completed("The selected companies were verified."); }, onError: failed }),
