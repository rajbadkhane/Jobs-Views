  ...
  return <EnterpriseCard title="Create Job" description="Create a complete draft from real employer input. Publish it after review from the jobs table." icon={<EditIcon />}>
    <form id="job-editor" className="grid scroll-mt-24 gap-4" onSubmit={async (event) => { event.preventDefault(); if (!companyId) return; setError(""); try { await live.actions.createJob.mutateAsync({ ...form, company_id: companyId, status: "draft" }); setForm(initial); } catch (caught) { setError(caught instanceof Error ? caught.message : "The job could not be created."); } }}>
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        <label className="grid gap-1.5 text-sm font-semibold">Job title<input required value={form.title} onChange={(event) => update("title", event.target.value)} className={formControl} /></label>
        <label className="grid gap-1.5 text-sm font-semibold">Job type<select value={form.job_type} onChange={(event) => update("job_type", event.target.value)} className={formControl}><option value="full_time">Full time</option><option value="part_time">Part time</option><option value="contract">Contract</option><option value="internship">Internship</option></select></label>
      ...
      <label className="grid gap-1.5 text-sm font-semibold">Full description<textarea required rows={7} value={form.full_description} onChange={(event) => update("full_description", event.target.value)} className={cn(formControl, "h-auto py-3")} /></label>
