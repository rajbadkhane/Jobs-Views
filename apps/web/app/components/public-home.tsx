...
import { ArrowRight, Bell, BookOpen, Briefcase, Building2, Calculator, ChevronDown, Menu, Moon, Search, Sparkles, Sun, UserRound, X } from "lucide-react";
...
export function Navbar() {
  ...
  useEffect(() => {
    const close = (event: MouseEvent) => { if (!root.current?.contains(event.target as Node)) { setMore(false); setMobile(false); } };
    const key = (event: KeyboardEvent) => { if (event.key === "Escape") { setMore(false); setMobile(false); } };
    document.addEventListener("mousedown", close);
    document.addEventListener("keydown", key);
    return () => { document.removeEventListener("mousedown", close); document.removeEventListener("keydown", key); };
  }, []);
  const close = () => { setMore(false); setMobile(false); };
  return (
    <header className="sticky top-0 z-50 border-b border-[var(--cos-outline-variant)] bg-[var(--cos-surface-container-lowest)]/95 backdrop-blur">
      <div ref={root} className={cn(container, "relative flex min-h-16 items-center gap-3 py-2")}>
        <Link href="/" onClick={close} aria-label="Jobs View home" className={cn("relative h-12 w-14 shrink-0", focus)}>
          <Image src="/images/logo-mark.png" fill sizes="56px" className="object-contain" alt="" priority />
        </Link>
        <nav className="hidden items-center gap-1 lg:flex" aria-label="Primary navigation">
          {publicRoutes.primary.map((route) => (
            <Link key={route.href} href={route.href} className={cn("rounded-md px-3 py-2 text-sm font-semibold hover:bg-[var(--cos-surface-container-low)]", focus)}>{route.label}</Link>
          ))}
          <div className="relative">
            <button onClick={() => setMore((value) => !value)} className={cn("inline-flex min-h-10 items-center gap-1 rounded-md px-3 text-sm font-semibold hover:bg-[var(--cos-surface-container-low)]", focus)} aria-expanded={more}>
              More <ChevronDown size={15} className={cn("transition-transform duration-200", more && "rotate-180")} />
            </button>
            {more ? (
              <div className="absolute left-0 top-12 z-[100] grid w-64 gap-1 rounded-xl border border-[var(--cos-outline-variant)] bg-[var(--cos-surface-container-lowest)] p-2 shadow-xl">
                {[...publicRoutes.explore, ...publicRoutes.tools].map((route) => (
                  <Link key={route.href + route.label} href={route.href} onClick={close} className="rounded-lg px-3 py-2 text-sm font-medium text-[var(--cos-on-surface)] hover:bg-[var(--cos-surface-container-low)]">{route.label}</Link>
                ))}
              </div>
            ) : null}
          </div>
        </nav>
        <button onClick={() => window.dispatchEvent(new CustomEvent("jobsview:open-command-center"))} className={cn("ml-auto hidden min-h-10 min-w-52 items-center gap-2 rounded-full border border-[var(--cos-outline-variant)] px-4 text-sm text-[var(--cos-on-surface-variant)] md:flex", focus)}>
        ...
        {user ? (
          <>
            <Link href="/candidate/notifications" aria-label="Notifications" className={cn("hidden rounded-md p-2 sm:inline-flex", focus)}><Bell size={18} /></Link>
            <Link href="/candidate" className={cn("hidden items-center gap-2 rounded-md px-3 py-2 text-sm font-bold sm:inline-flex", focus)}><UserRound size={17} /> My account</Link>
          </>
        ) : (
          <>
            <Link href="/login" className="hidden px-2 py-2 text-sm font-bold sm:block">Login</Link>
            <Link href="/register" className="hidden rounded-md bg-[var(--cos-primary)] px-4 py-2 text-sm font-bold text-white md:block">Sign up free</Link>
          </>
        )}
        ...
        {mobile ? (
          <div className="fixed inset-0 z-50 bg-slate-950/45">
            <aside className="ml-auto flex h-dvh w-[min(90vw,380px)] flex-col bg-[var(--cos-surface-container-lowest)] p-4">
              <div className="flex items-center justify-between"><strong>Menu</strong><Button variant="ghost" size="icon" aria-label="Close navigation" onClick={close}><X size={18} /></Button></div>
              <nav className="mt-4 grid gap-1 overflow-auto">
                <LanguageSwitcher />
                {publicRoutes.primary.map((route) => <Link key={route.href + route.label} href={route.href} onClick={close} className="min-h-11 flex items-center rounded-md px-3 font-semibold hover:bg-[var(--cos-surface-container-low)]">{route.label}</Link>)}
                <div className="text-xs font-bold text-[var(--cos-on-surface-variant)] px-3 py-2 mt-2">Browse by category</div>
                {publicRoutes.explore.map((route) => <Link key={route.href + route.label} href={route.href} onClick={close} className="min-h-11 flex items-center rounded-md px-3 font-medium hover:bg-[var(--cos-surface-container-low)]">{route.label}</Link>)}
                <div className="text-xs font-bold text-[var(--cos-on-surface-variant)] px-3 py-2 mt-2">Tools</div>
                {publicRoutes.tools.map((route) => <Link key={route.href + route.label} href={route.href} onClick={close} className="min-h-11 flex items-center rounded-md px-3 font-medium hover:bg-[var(--cos-surface-container-low)]">{route.label}</Link>)}
              </nav>
              <div className="mt-auto grid gap-2 border-t border-[var(--cos-outline-variant)] pt-4">
                <Link href="/login" onClick={close} className="rounded-md border p-3 text-center font-bold">Login</Link>
                <a href={publicRoutes.employer.href} className="rounded-md bg-[var(--cos-primary)] p-3 text-center font-bold text-white">I want to hire</a>
              </div>
            </aside>
          </div>
        ) : null}
...
function Hero() {
  ...
  return (
    <section className="border-b border-[var(--cos-outline-variant)] bg-[var(--cos-surface-container-low)]">
      <div className={cn(container, "grid items-center gap-10 py-12 lg:grid-cols-[1.1fr_.9fr]")}>
        <Reveal>
          <h1 className="max-w-3xl text-4xl font-extrabold leading-tight sm:text-5xl text-[var(--cos-on-surface)]">
            Find your next job
          </h1>
          <p className="mt-4 max-w-2xl text-lg leading-8 text-[var(--cos-on-surface-variant)]">
            Search jobs for 10th pass, 12th pass, ITI, freshers, and experienced workers across India. Free to search and apply.
          </p>
          <form onSubmit={submit} className="mt-6 grid gap-2 rounded-xl border border-[var(--cos-outline-variant)] bg-[var(--cos-surface-container-lowest)] p-3 sm:grid-cols-[1fr_1fr_auto]">
            <label className="grid gap-1">
              <span className="text-xs font-bold text-[var(--cos-on-surface-variant)]">What job?</span>
              <input value={q} onChange={(event) => setQ(event.target.value)} className="h-12 min-w-0 rounded-lg border border-[var(--cos-outline-variant)] bg-transparent px-3.5 font-semibold outline-none focus:border-[var(--cos-primary)] focus:ring-2 focus:ring-[var(--cos-primary)]/20" placeholder="Driver, nurse, accountant..." />
            </label>
            <label className="grid gap-1">
              <span className="text-xs font-bold text-[var(--cos-on-surface-variant)]">Where?</span>
              <input value={location} onChange={(event) => setLocation(event.target.value)} className="h-12 min-w-0 rounded-lg border border-[var(--cos-outline-variant)] bg-transparent px-3.5 font-semibold outline-none focus:border-[var(--cos-primary)] focus:ring-2 focus:ring-[var(--cos-primary)]/20" placeholder="City or Remote" />
            </label>
            <Button variant="gradient" className="self-end sm:h-12 px-6 font-bold"><Search size={18} /> Search jobs</Button>
          </form>
          <div className="mt-4 flex flex-wrap gap-2">
            {["10th pass", "12th pass", "ITI", "Fresher", "Experienced", "Nursing", "Work from home", "Remote"].map((item) => (
              <Link key={item} href={`/jobs?q=${encodeURIComponent(item)}`} className="rounded-full border border-[var(--cos-outline-variant)] bg-[var(--cos-surface-container-lowest)] px-3.5 py-1.5 text-sm font-semibold hover:border-[var(--cos-primary)] hover:text-[var(--cos-primary)]">
                {item}
              </Link>
            ))}
          </div>
        ...
        <Reveal>
          <div className="relative mx-auto aspect-[4/3] w-full max-w-xl overflow-hidden rounded-xl border border-[var(--cos-outline-variant)]">
            <Image src="/images/home-hero-india-careers.png" fill priority sizes="(max-width:1023px) 100vw, 45vw" className="object-cover" alt="People looking for jobs" />
          </div>
        </Reveal>
      </div>
    </section>
...
function AudiencePaths(){return <Section title="What do you want to do?"><div className="grid gap-6 md:grid-cols-2"><PathCard icon={<Search/>} title="I'm looking for a job" text="Search jobs for free. You only need to sign in when you apply." href="/jobs" action="Browse jobs"/><PathCard icon={<Building2/>} title="I want to hire people" text="Post a job and start receiving applications from candidates." href={publicRoutes.employer.href} action="Go to employer portal"/></div></Section>}
function LatestJobs(){
...
function EntryRoles(){return <Section title="Popular job categories" description="Pick a category to see jobs like this."><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{roleCards.map(([title,query,description])=><Link key={query} href={`/jobs?q=${encodeURIComponent(query)}`} className={cn("flex flex-col justify-between rounded-xl border border-[var(--cos-outline-variant)] bg-[var(--cos-surface-container-lowest)] p-5 hover:border-[var(--cos-primary)]",focus)}><div className="flex items-center gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[var(--cos-primary)]/10 text-[var(--cos-primary)]"><Briefcase size={18}/></span><h3 className="font-bold text-[var(--cos-on-surface)]">{title}</h3></div><p className="mt-3 text-sm leading-6 text-[var(--cos-on-surface-variant)]">{description}</p><span className="mt-4 inline-flex items-center gap-1.5 text-sm font-bold text-[var(--cos-primary)]">See jobs <ArrowRight size={15}/></span></Link>)}</div></Section>}
function PopularEmployers(){const query=useQuery({queryKey:["home","companies"],queryFn:()=>companyApi.search({limit:12})});const companies=asItems<PublicCompany>(query.data);return <Section title="Companies hiring now" description="Some of the companies posting jobs on Jobs View." action={{label:"See all companies",href:"/companies"}}><div className="overflow-hidden rounded-md border border-[var(--cos-outline-variant)] py-4"><div className="jobsview-partner-track flex w-max gap-3 px-3">{[...employers,...employers].map((name,index)=>{const match=companies.find((company)=>company.name.toLowerCase()===name.toLowerCase());const href=match?`/companies/${match.slug}`:`/jobs?company=${encodeURIComponent(name)}`;return <Link key={`${name}-${index}`} href={href} className="flex h-20 w-44 shrink-0 items-center gap-3 rounded-md border border-[var(--cos-outline-variant)] bg-[var(--cos-surface-container-lowest)] px-4 hover:border-[var(--cos-primary)]"><ResilientImage src={`/images/employers/${logoName(name)}.${logoExtension(name)}`} alt="" fallbackLabel={name} wrapperClassName="h-11 w-11 shrink-0 rounded-md" className="object-contain"/><span className="font-bold">{name}</span></Link>})}</div></div>{query.isError?<p className="mt-3 text-sm text-[var(--cos-on-surface-variant)]">Company list is temporarily unavailable. You can still search jobs by company name.</p>:null}</Section>}
function CareerTools(){const tools=[{icon:<Calculator/>,title:"Salary calculator",text:"Check what a job usually pays before you apply.",href:"/salary/calculator"},{icon:<BookOpen/>,title:"Resume builder",text:"Make a simple, ready-to-send resume in minutes.",href:"/resume-builder"},{icon:<UserRound/>,title:"Career guidance",text:"Get simple tips on choosing and growing your career.",href:"/guidance"}];return <Section title="Free tools to help you" description="No sign-up needed to try these."><div className="grid gap-5 md:grid-cols-3">{tools.map((tool)=><PathCard key={tool.title} {...tool} action="Open"/>)}</div></Section>}
function EmployerCTA(){return <section className="border-t border-b border-[var(--cos-outline-variant)] bg-[var(--cos-primary)] text-white"><div className={cn(container,"flex flex-col items-start justify-between gap-6 py-12 md:flex-row md:items-center")}><div><span className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-white/80 mb-2"><Building2 size={15}/> For employers</span><h2 className="text-2xl font-extrabold sm:text-3xl">Need to hire people?</h2><p className="mt-2 max-w-2xl text-base text-white/85">Post a job in a few minutes and start getting applications.</p></div><a href={publicRoutes.employer.href} className="rounded-lg bg-white px-7 py-3.5 text-base font-bold text-[var(--cos-primary)] hover:bg-white/90">Post a job</a></div></section>}
...
function Section({title,description,action,children}:{title:string;description?:string;action?:{label:string;href:string};children:React.ReactNode}){return <Reveal><section className={cn(container,"py-10 sm:py-14")}><div className="mb-6 flex flex-wrap items-end justify-between gap-4"><div><h2 className="text-2xl font-bold sm:text-3xl text-[var(--cos-on-surface)]">{title}</h2>{description?<p className="mt-2 max-w-3xl text-base text-[var(--cos-on-surface-variant)]">{description}</p>:null}</div>{action?<Link href={action.href} className="inline-flex items-center gap-1 font-bold text-[var(--cos-primary)] hover:underline">{action.label}<ArrowRight size={16}/></Link>:null}</div>{children}</section></Reveal>}
...
function PathCard({icon,title,text,href,action}:{icon:React.ReactNode;title:string;text:string;href:string;action:string;tone?:string}){const external=href.startsWith("http");const body=<><span className="inline-flex h-11 w-11 items-center justify-center rounded-lg bg-[var(--cos-primary)]/10 text-[var(--cos-primary)]">{icon}</span><h3 className="mt-4 text-lg font-bold text-[var(--cos-on-surface)]">{title}</h3><p className="mt-2 text-sm leading-6 text-[var(--cos-on-surface-variant)]">{text}</p><span className="mt-4 inline-flex items-center gap-1.5 font-bold text-[var(--cos-primary)]">{action}<ArrowRight size={16}/></span></>;const classes=cn("rounded-xl border border-[var(--cos-outline-variant)] bg-[var(--cos-surface-container-lowest)] p-6 hover:border-[var(--cos-primary)]",focus);return external?<a href={href} className={classes}>{body}</a>:<Link href={href} className={classes}>{body}</Link>}
